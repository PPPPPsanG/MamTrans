...
class Conv(nn.Module):
    ...
    def __init__(self, dim: int, hidden_dim: int, num_patch: int, patch_size: int):
        ...
        self.linear1 = nn.Sequential(
            ...
            nn.Linear(dim, 64))#, kernel_size=1, padding=0, bias=False))
        ...
        self.linear2 = nn.Sequential(
            ...
            nn.Linear(64, dim))# kernel_size=1, padding=0, bias=False))
...
class Encoder_Transformer(nn.Module):
    ...
    def __init__(self, dim: int, num_layers: int, num_heads: int, head_dim: int, hidden_dim: int, num_patch: int, patch_size: int):
        ...
        for _ in range(num_layers):
            layer = [
                # nn.Sequential(nn.LayerNorm(dim), Mamba(d_model = dim),),
                nn.Sequential(nn.LayerNorm(dim), Att(dim, head_dim, num_heads, num_patch, patch_size)),
                nn.Sequential(nn.LayerNorm(dim), Conv(dim, hidden_dim, num_patch, patch_size))
            ]
